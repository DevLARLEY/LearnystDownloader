...
class Learnyst:
    ...
    def _download_manifest(
        ...
        with open(manifest, "r") as file:
            manifest_json = xmltodict.parse(file.read())
        ...
        for ad_set in ensure_list(manifest_json["MPD"]["Period"]["AdaptationSet"]):
            if ad_set["@contentType"] in ("video", "audio"):
                ...
                name = representation[0].get('BaseURL')  # Assign 'BaseURL' to 'name'
...
if __name__ == "__main__":
    ...
    parser = ArgumentParser(
        prog="Learnyst Downloader",
        description="Author: github.com/DevLARLEY"
    )
    parser.add_argument(
        "--url",
        type=str,
        help="Full course URL",
        required=True
    )
    parser.add_argument(
        "--debug", "--d",
    ...
    if not (lr_token := cf.simple_get("lrToken")):
        ...
        player_manager = PlayerManager(
            ...
            version=api_version,
            lc=lc,
            player_file='player.js'
        )
        handle(player_manager.get_player(), "Unable to request player")
        handle(player_manager.inject_exports(), "Unable to inject exports")

    logging.info("Verifying token...")
    if not is_token_valid(lr_token):
        cf.simple_set("lrToken", None)
        handle(False, "Token expired, please re-enter")
